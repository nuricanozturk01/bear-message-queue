...
// node_modules/mermaid/dist/chunks/mermaid.core/chunk-HKLBG3BQ.mjs
var labelHelper = __name((parent, node, _classes) => __async(null, null, function* () {
  let cssClasses;
...
var insertLabel = __name((parent, label, options) => __async(null, null, function* () {
...
function addHtmlLabel(node) {
  return __async(this, null, function* () {
    ...
    if (node.label && hasKatex(node.label)) {
      label = yield renderKatex(node.label.replace(common_default.lineBreakRegex, "\n"), getConfig2());
    }
...
var createLabel = __name((_vertexText, style, isTitle, isNode) => __async(null, null, function* () {
  ...
  if (evaluate(getConfig2().flowchart.htmlLabels)) {
    ...
    let vertexNode = yield addHtmlLabel(node);
    return vertexNode;
  } else {
...
var roundedWithTitle = __name((parent, node) => __async(null, null, function* () {
  const siteConfig = getConfig2();
  const { themeVariables, handDrawnSeed } = siteConfig;
  ...
  const text2 = label.node().appendChild(yield createLabel_default(node.label, node.labelStyle, void 0, true));
  let bbox = text2.getBBox();
  if (evaluate(siteConfig.flowchart.htmlLabels)) {
    const div = text2.children[0];
    const dv = select_default(text2);
...
var kanbanSection = __name((parent, node) => __async(null, null, function* () {
  log.info("Creating subgraph rect for ", node.id, node);
  const siteConfig = getConfig2();
  const { themeVariables, handDrawnSeed } = siteConfig;
  const { clusterBkg, clusterBorder } = themeVariables;
...
var insertCluster = __name((elem, node) => __async(null, null, function* () {
...
function rectWithTitle(parent, node) {
  return __async(this, null, function* () {
    ...
    const text2 = label.node().appendChild(yield createLabel_default(title, node.labelStyle, true, true));
...
function roundedRect(parent, node) {
  return __async(this, null, function* () {
    const options = {
      rx: 5,
      ry: 5,
...
function tiltedCylinder(parent, node) {
  return __async(this, null, function* () {
    const { labelStyles, nodeStyles } = styles2String(node);
    node.labelStyle = labelStyles;
    const { shapeSvg, bbox, label, halfPadding } = yield labelHelper(
      parent,
      node,
      getNodeClasses(node)
    );
...
function textHelper(_0, _1, _2, _3) {
  return __async(this, arguments, function* (parent, node, config, useHtmlLabels, GAP = config.class.padding ?? 12) {
    ...
    if (node.annotations.length > 0) {
      ...
      yield addText(annotationGroup, { text: `«${annotation}»` }, 0);
      const annotationGroupBBox = annotationGroup.node().getBBox();
      annotationGroupHeight = annotationGroupBBox.height;
    }
    labelGroup = shapeSvg.insert("g").attr("class", "label-group text");
    yield addText(labelGroup, node, 0, ["font-weight: bolder"]);
    const labelGroupBBox = labelGroup.node().getBBox();
    labelGroupHeight = labelGroupBBox.height;
    membersGroup = shapeSvg.insert("g").attr("class", "members-group text");
    let yOffset = 0;
    for (const member of node.members) {
      const height = yield addText(membersGroup, member, yOffset, [member.parseClassifier()]);
    ...
    for (const method of node.methods) {
      const height = yield addText(methodsGroup, method, methodsYOffset, [method.parseClassifier()]);
...
//# sourceMappingURL=chunk-66JO2TML.js.map

...
public class MetricsService {
  ...
  public MetricsSummaryDto summary() {
    ...
    return new MetricsSummaryDto(usedVhosts, usedQueues, usedExchanges);
  }
  ...
  public VhostMetricsDto vhostMetrics(final String vhostId) {
    ...
    final int bindingCount =
        (int) this.bindingRepository.findAllActiveForReadByVhostId(vhostId).size();

    final boolean vhostLoaded = this.brokerServerFacade.isVhostLoaded(vhostId);
    final Set<String> loaded = this.brokerServerFacade.getLoadedQueueNames(vhostId);
            ...
            .map(
                q ->
                    new QueueMetricDto(
                        q.getId(),
                        q.getName(),
                        q.getStatus() != null ? q.getStatus().name() : "UNKNOWN",
                        loaded.contains(q.getName())))
            .toList();

    return new VhostMetricsDto(
        vhost.getId(),
        vhost.getName(),
        vhostLoaded,
        queues.size(),
